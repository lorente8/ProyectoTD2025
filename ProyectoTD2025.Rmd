...
```{r}
df_producto <- df %>% select(c(fs, productos)) %>% 
  separate_rows(productos, sep = ";")
```


No vamos a analizar los datos de listo para comer asi que los eliminamos(En teoria no deberian de haber pero eliminamos por si acaso)
```{r}
#Eliminar línea listo comer
listo <- which(df_producto$productos == "LISTO COMER")
df_produc <- df_produc[-(listo),]

```


Primero trabajamos con los productos genericos y despues veremos como analizar pescado, verdura y fruta que es vendidia al kg.
