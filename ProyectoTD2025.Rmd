...
# Verificación
cat("Se eliminarán", length(filas_fruta_verdura), "filas de fruta/verdura\n")
print("Ejemplo de filas a eliminar:")
print(df_sin_pescado[filas_fruta_verdura[1:2], ]) # Muestra primeros 2 pares
...
df_productos_unidades <- df_resto %>%
  mutate(
    ...
    importe = as.numeric(gsub(",", ".", str_extract(productos, "\\d+,\\d+$"))),
    
    # Método alternativo usando expresiones regulares
    descripcion = gsub(
      pattern = "^\\d+\\s+|\\s+\\d+,?\\d*$", 
      replacement = "", 
      x = productos
  ...
  select(num_ticket, descripcion, cantidad, precio_unitario, importe)
head(df_productos_unidades)
```
