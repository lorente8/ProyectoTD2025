...
procesar_linea <- function(i) {
  ...
  if (length(linea) > 0) {
    ...
    # Si la línea está vacía, detener el procesamiento
    if (linea_trimmed == "") {
      return(FALSE)
    }
...
# Comenzar a procesar desde la línea 12 y continuar hasta encontrar una línea vacía
i <- 12
while (i <= length(todas_las_lineas)) {
  if (!procesar_linea(i)) {
    break  # Detener el procesamiento cuando se encuentra una línea vacía
  }
  i <- i + 1
}
